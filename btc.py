...
def main():
    ...
    print(f"🌐 التشغيل على Render: {ON_RENDER}")
    ...
    if test_msg:
        ...
        # على Render، لا نوقف البرنامج تماماً
        if ON_RENDER:
            print("⚠️ المتابعة رغم خطأ Telegram (للتشغيل على Render)")
    ...
    send_telegram_message("🔄 <b>تم بدء النظام على Render بنجاح!</b>")
    
    # الحلقة الرئيسية - مع تحسينات للعمل على Render
    while True:
        try:
            schedule.run_pending()
            # تقليل استهلاك الموارد على Render
            time.sleep(30)  # انتظار 30 ثانية بدلاً من 1
        except Exception as e:
            print(f"❌ خطأ غير متوقع: {e}")
            # على Render، نستمر رغم الأخطاء
            time.sleep(60)
